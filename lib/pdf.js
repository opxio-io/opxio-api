...
async function drawHeader(doc, F, { logoUrl, coName, coContact, docTitle }) {
  ...
  if (logoUrl) {
    try {
      ...
      if (resp.ok) {
        ...
        doc.image(buf, MARGIN + mm(6), y0 + mm(4),
  ...
  if (coContact) {
    doc.font(F.reg).fontSize(8).fillColor(C.subtle)
      .text(coContact, MARGIN + mm(5), y0 + mm(20), { width: USABLE * 0.5, lineBreak: false })
  }
...
export async function generateQuotationPdf(data) {
  ...
  const validDate  = data.valid_until || (issueDate
...
export async function fetchQuotationData(pageId, token) {
  ...
  const currency     = props["Currency"]?.select?.name || ""
  ...
  const companyRels = props.Client?.relation || props.Company?.relation || []
  ...
  return {
    ...
    currency,
    valid_until:     props["Valid Until"]?.date?.start || "",
  }
}
